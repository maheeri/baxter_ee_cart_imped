...
/**
* Callback to store the JointState in a global variable 
*/
void jointStateCallback (const sensor_msgs::JointState::ConstPtr& jointStateMsg) {
  baxter_joint_state = *jointStateMsg;
} 

// Main is needed for baxter porting 
int main(int argc, char** argv) {
  std::string urdf_file = "/baxter_description/urdf/baxter.urdf";
  
  if (!baxter_model.initFile(urdf_file)) {
  	ROS_ERROR("Failed to parse urdf file");
  	return -1;
  }
  
  ROS_INFO("Successfully parsed baxter urdf file");
  
  if (!kdl_parser::treeFromUrdfModel(baxter_model, baxter_tree)) {
